...
subspec2 = gridspec.GridSpecFromSubplotSpec(24, 1,subplot_spec=spec[21:,6:], wspace=-0.5, hspace=0.0)
...
#row5
#f1_ax15 = fig.add_subplot(subspec1[:,:])
...
def animate(i, data_x, data_y, data_x2, data_y2):
	...
	####
	f1_ax16.text(0.5,0.5, f"{len(avg_defocus)}")
	...
	if 'rln_directoy/Class2D' :
		...
		if os.path.isfile(path_to_last_strip):	
			...
			for ri, ci, i in zip(row_inx, col_inx, range(0,100)):
				...
				ax.imshow(c2d_flat[:,:,i], cmap='coolwarm')
				ax.set_xticks([])
				ax.set_yticks([])
		else: 
