...
class RelionItGui(object):
    ...
    def __init__(self, main_window, options):
        ...
        tk.Label(self.prep_frame, text=u"(Super-res) pixel size (A):").grid(row=row, sticky=tk.W)
        ...
        tk.Label(self.prep_frame, text=u"Exposure rate (e/A**2/frame):").grid(row=row, sticky=tk.W)
        ...
        tk.Label(self.particle_frame, text=u"Particle diameter (A):").grid(row=row, sticky=tk.W)
        ...
        tk.Label(self.particle_frame, text=u"Mask diameter (A):").grid(row=row, sticky=tk.W)
    ...
    def save_options_from_gui(self):
        ...
        try:
            ...
            if len(warnings) == 0 or messagebox.askokcancel("Warning", "\n".join(warnings), icon='warning',
